...
export const WishlistCard: React.FC<WishlistCardProps> = ({
  ...
  return (
    <div className="group relative bg-white/5 backdrop-blur-sm border border-gray-700/30 rounded-2xl overflow-hidden hover:bg-white/10 hover:border-gray-600/50 transition-all duration-300 hover:shadow-2xl hover:scale-105">
      <div className="aspect-[2/3] relative overflow-hidden">
        ...
        {/* Type Badge */}
        <div className={`absolute top-3 left-3 px-2.5 py-1.5 bg-gradient-to-r ${getTypeColor(item.type)} rounded-full text-white text-xs font-medium flex items-center gap-1.5 shadow-lg`}>
        ...
        <div className="absolute top-3 right-3 flex gap-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
          {showMoveButton && onMove && (
            <button
              ...
              aria-label={moveButtonText || (moveButtonIcon === 'right' ? 'Mark as watched' : 'Move back to watchlist')}
              className={`p-3 md:p-2.5 ${moveButtonIcon === 'right' ? 'bg-green-500/90 hover:bg-green-500' : 'bg-blue-500/90 hover:bg-blue-500'} text-white rounded-full shadow-lg transition-all duration-200 hover:scale-110`}
          ...
          <button
            ...
            className="p-3 md:p-2.5 bg-red-500/90 hover:bg-red-500 text-white rounded-full shadow-lg transition-all duration-200 hover:scale-110"
      ...
      <div className="p-5">
        <h3 className="font-bold text-white mb-3 line-clamp-2 group-hover:text-purple-300 transition-colors duration-200 text-lg">
          {item.title}
        </h3>
